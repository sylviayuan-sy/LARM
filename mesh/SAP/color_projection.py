...
class Pix2FacesRenderer:
    ...
    def render_pix2faces_nvdiff(self, meshes: Meshes, cameras: CamerasBase, H=512, W=512, return_rast=False):
...
def project_color(meshes: Meshes, cameras: CamerasBase, pil_image: Image.Image, use_alpha=True, eps=0.05, resolution=1024, device="cuda") -> dict:
    ...
    world_points = cameras.unproject_points(torch.tensor([[[0., 0., 0.1], [0., 0., 0.2]]]).to(device), from_ndc=True)[0]
    ...
    # assert cos_angles.mean() < 0, f"The view direction is not correct. {cos_angles.mean()=} {world_points[0]}"
    ...
    # compute color
    pt_tensor = cameras.transform_points_ndc(verts_coordinates)[..., :2] # NDC space points
