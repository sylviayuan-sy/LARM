...
# capture idx and optional qpos like 0_1, 0_1.0, 0_1.00, 0_-0.25, etc.
...
def _list_frame_stems(images_dir: str, max_views=None, filter_qpos: float = None, eps: float = 1e-6):
    """
    Return [(frame_idx, stem), ...], optionally filtering by qpos (float comparison with tolerance).
    Skips files without qpos when filter_qpos is not None.
    """
    items = []
    for fn in os.listdir(images_dir):
        ...
        if filter_qpos is not None:
            if qpos_s is None:
                # no qpos in name -> skip when filtering
                continue
            try:
...
def process_single(base_dir, out_dir, num_views=4, qpos_only: float = 1.0):
    ...
    for frame_idx, stem in frame_items:
        ...
        depth = np.load(depth_path).reshape(-1) * 5.0
...
def _worker(args):
    base_dir, out_dir, num_views, qpos_only = args
    try:
        process_single(base_dir, out_dir, num_views=num_views, qpos_only=qpos_only)
        return True
    except Exception as e:
        return f"{os.path.basename(base_dir)}: {e}"

def main():
    parser = argparse.ArgumentParser(description="Process pointclouds with masks and transforms")
    parser.add_argument('--load_dir', type=str, default="./output",
                        help="Root containing eval_*_joint_* subfolders.")
    parser.add_argument('--out_dir', type=str, default=None,
                        help="(Optional) Where to write sap_in pointclouds. If not set, defaults to <load_dir>/sap_in.")
    ...
    parser.add_argument('--qpos_only', type=float, default=1.0,
                        help="Only process frames whose filename qpos equals this value (e.g., 1.0).")
    ...
    print(f"[info] qpos_only = {args.qpos_only}")

    subs = [
        os.path.join(args.load_dir, d)
        for d in os.listdir(args.load_dir)
        if os.path.isdir(os.path.join(args.load_dir, d)) and VALID_SUB_RE.match(d)
    ]
    num_views = None if args.num_views == -1 else args.num_views
    jobs = [(sub, out_dir, num_views, args.qpos_only) for sub in subs]
