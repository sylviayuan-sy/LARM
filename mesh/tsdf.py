...
def _integrate_and_save(part_name: str,
                        ...
                        save_debug: bool) -> Optional[str]:
    """
    Integrate only frames listed in `images`:
    - depth is multiplied by 5.0
    ...
    for idx, _, fn in images:
        ...
        depth_masked = (np.load(depth_path).astype(np.float32) * 5.0) * cur_mask  # *** scale to meters ***
...
def process_one(path: str,
    ...
    base_clean_ply = _integrate_and_save("base", images, frames_meta, images_dir, meta, path, file_name, qstr, min_component_faces, save_debug)
    part_clean_ply = _integrate_and_save("part", images, frames_meta, images_dir, meta, path, file_name, qstr, min_component_faces, save_debug)
    ...
    mbase_ply = _integrate_and_save("multilink_base", images, frames_meta, images_dir, meta, path, file_name, qstr, min_component_faces, save_debug)
...
def process_entry(load_folder: str,
    ...
    if not os.path.isdir(path):
        ...
        return
    process_one(path, file_name, max_frames, min_component_faces, save_debug, qpos_only)
