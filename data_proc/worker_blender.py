...
POSE_COUNT = 25             # length of pose list when generating randomly, also number of poses to render
NUM_INPUTS = 32             # number of input images
...
def build_pose_sequence() -> list[float]:
    ...
    Build the joint pose list:
      ...
      - Otherwise generate random samples in (0,1) excluding endpoints, always start with 1.0 and end with 0.0.
    """
    if POSE_LIST_OVERRIDE:
        if len(POSE_LIST_OVERRIDE) <= 2:
            seq = sorted(POSE_LIST_OVERRIDE, reverse=True)
        else:
            sorted_desc = sorted(POSE_LIST_OVERRIDE, reverse=True)
            largest = sorted_desc[0]
            smallest = sorted_desc[-1]
            middle = sorted_desc[1:-1]  # Already in descending order
            seq = [largest, smallest] + middle
        return seq
    ...
    # Sample from (0, 1) exclusive - avoid exact 0.0 and 1.0
    middle = []
    while len(middle) < mid_n:
        sample = np.random.rand()
        # Exclude values that round to 0.0 or 1.0 (with small epsilon)
        if 0.001 < sample < 0.999:
            middle.append(sample)
    
    middle.sort(reverse=True)  # descending order
    return [1.0] + middle + [0.0]


def read(x: str, rot_aug: bool = False):
    ...
    for t in range(NUM_TEX):
        ...
        # Generate intrinsics once per texture - shared across all joints and poses
        intrinsics, fovy = generate_intrinsics(rot_aug=rot_aug)
        scaled_pose_list = {
        ...
        for joint in joints:
            ...
            # Pose sequence (either override list as-is; or random with [0.0,...,1.0])
...
def generate_intrinsics(rot_aug=False):
    if rot_aug:
        fovy = np.radians(np.random.uniform(45, 65))
    else:
...
def render(x: str, output_dir: str, args):
    ...
    joint_filter = getattr(args, 'joint_filter', None)  # Get joint filter list
    
    if read(x, rot_aug=rot_aug) == -1:
        return
    ...
    # -------------------------------------------------------
    # Detect parent object folder for consistent HDRI/lighting
    # Note: Intrinsics are already stored in the JSON from read() function
    # -------------------------------------------------------
    ...
    if parent_obj not in OBJECT_CONFIGS:
        ...
        print(f"[INFO] Reusing lighting config for {parent_obj}")
    ...
    for tex, tex_data in enumerate(scaled_pose_list):
        for joint, joint_data in tex_data.items():
            ...
            # Filter by joint if joint_filter is provided
            if joint_filter is not None and joint not in joint_filter:
                print(f"[INFO] Skipping joint {joint} (not in filter list)")
                continue
            ...
            for scale, poses in joint_data.items():
                ...
                rot_aug_flag = " --rot-aug" if rot_aug else ""
                base_cmd = (
                    ...
                    f"--out_normal --out_depth --color_ind {color_inds}{rot_aug_flag}"
                )

                available = len(poses.get("pose_list", []))
                to_render = min(POSE_COUNT, max(1, available))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--datalist_path", type=str, required=True, help="Path to datalist file (one file per line)")
    parser.add_argument("--output_dir", type=str, required=True, help="Output directory for renderings")
    parser.add_argument("--pose-count", type=int, default=8, help="Number of poses when generating randomly (also number to render)")
    parser.add_argument("--num-inputs", type=int, default=32, help="Number of input images")
    ...
    parser.add_argument("--rot-aug", action="store_true",
                        help="Use rotation augmentation mode: fovy sampled from 45-65 degrees, radius_scale = 0.8")
    ...
    with open(args.datalist_path, "r") as f:
        for line in f:
            ...
            if not line:
                continue
            basename = os.path.basename(line).replace(".json", "")
            parts = basename.split("_joint_")
            if len(parts) == 2:
                obj_id = parts[0]
                joint_suffix = parts[1]
                if obj_id not in target_objects:
                    target_objects[obj_id] = []
                target_objects[obj_id].append(f"joint_{joint_suffix}")

    print(f"[INFO] Found {len(target_objects)} objects to process")
    for obj_id, joints in target_objects.items():
        print(f"[INFO] Object {obj_id}: joints {joints}")

    for path in glob(os.path.join(args.data_root, "*")):
        obj_id = os.path.basename(path)
        if obj_id in target_objects:
            # Pass the joint filter to render
            args.joint_filter = target_objects[obj_id]
